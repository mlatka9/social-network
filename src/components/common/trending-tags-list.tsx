...
const TrendingTagsList = () => {
  ...
  return (
    <aside className=" bg-white rounded-xl sticky h-fit top-[92px] dark:bg-slate-800">
      <TextHeader className=" py-3 px-5 ">Trends for you</TextHeader>
      <hr className=" px-5 " />
      {isSuccess &&
        data.map((tagData) => (
          <Link href={`/tag/${tagData.tagName}`} key={tagData.tagName}>
            <div className=" cursor-pointer hover:bg-blue-50 hover:dark:bg-slate-900 px-5 py-3">
